-- Complete Fix for Parent Signup Issue
-- Run this ENTIRE file in your Supabase SQL Editor
-- This will fix the database error when parents try to sign up

-- Step 1: Make player-specific fields nullable (they're only required for players, not parents)
...
-- Step 2: Update handle_new_user function to properly handle both parents and players
...
BEGIN
  ...
  IF user_role = 'parent' THEN
    -- For parents, insert with parent-specific fields and NULL for player fields (since they're now nullable)
...
-- Step 3: Verify the trigger exists and is properly set up
...
-- SELECT column_name, is_nullable FROM information_schema.columns WHERE table_name = 'profiles' AND column_name IN ('player_name', 'program_type', 'competitive_level');
