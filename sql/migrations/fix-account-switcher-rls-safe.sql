-- Fix RLS Policy for Account Switcher (SAFE VERSION)
...
-- Step 1: Verify current policies exist (safety check)
DO $$
DECLARE
  parent_policy_exists BOOLEAN;
  player_policy_exists BOOLEAN;
BEGIN
  ...
  -- Check if player policy exists
  SELECT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'parent_player_relationships' 
    AND policyname = 'Players can view their relationships'
  ) INTO player_policy_exists;
  ...
  -- Note: Player policy might not exist if this is first run, which is okay
END $$;

-- Step 2: Drop existing function if it exists (safe - will recreate)
DROP FUNCTION IF EXISTS public.get_player_parent_id(UUID);

-- Step 3: Create a helper function to get player's parent_id (avoids recursion)
...
-- Step 4: Drop existing player policy (must be done before creating new one)
...
-- 1. Relationships where they are the player (their own relationship) - BACKWARD COMPATIBLE
-- 2. Relationships where they share the same parent (for account switcher) - NEW FEATURE
